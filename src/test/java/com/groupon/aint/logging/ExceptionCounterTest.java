...
public class ExceptionCounterTest {
    ...
    public void nonEmptyFile() throws Exception {
        ...
        MetricsAppenderTests.attachAppenderToRootLogger(new ExceptionCounter(metricsFactory));

        final Logger LOGGER = LoggerFactory.getLogger(EventLevelCounterTest.class);
        LOGGER.info().setThrowable(new NullPointerException("testing")).log();
        ...
        Assert.assertTrue(lines.get(0).contains("\"counters\":{\"logging/exceptions/NullPointerException\":{\"values\":[{\"value\":1}]}}"));
    }
